...
<div class="breadcrumb-line breadcrumb-line-light header-elements-md-inline">
	<div class="d-flex">
		<div class="breadcrumb">
			...
			<span class="breadcrumb-item active">Biller Provider</span>
		</div>
	</div>
</div>
@endsection
...
<div class="card">
	<div class="card-header header-elements-inline">
        <h5 class="card-title">List Biller Provider</h5>
			...
			<form @submit.prevent="editMode ? updateData() : storeData()" @keydown="form.onKeydown($event)">
				<div class="modal-body">
                    <div class="form-group">
                        ...
                        <input id="provider_name" type="text" placeholder="Provider Name" class="form-control"
                            v-model="form.provider_name" :class="{ 'is-invalid': form.errors.has('provider_name') }">
    ...
    var app = new Vue({
        ...
        methods: {
            ...
            storeData(){
                ...
                this.form.post("{{ route('billerprovider.store') }}")
                    .then(response => {
            ...
            updateData(){
                ...
                url = "{{ route('billerprovider.update', ':id') }}".replace(':id', this.form.id)
                this.form.put(url)
                    .then(response => {
            ...
            deleteData(id){
                DeleteConfirmation.fire().then((result) => {
                    if (result.value) {
                        url = "{{ route('billerprovider.destroy', ':id') }}".replace(':id', id)
                        axios.delete(url)
                            .then(response => {
            ...
            refreshDatatable(){
            	axios.get("{{ route('billerprovider.all') }}")
                    .then(response => {
