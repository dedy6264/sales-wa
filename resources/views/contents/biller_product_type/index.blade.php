...
<div class="breadcrumb-line breadcrumb-line-light header-elements-md-inline">
	<div class="d-flex">
		<div class="breadcrumb">
			...
			<span class="breadcrumb-item active">Biller Product Type</span>
		</div>
	</div>
</div>
@endsection
...
<div class="card">
	<div class="card-header header-elements-inline">
        <h5 class="card-title">List Product Type</h5>
			...
			<form @submit.prevent="editMode ? updateData() : storeData()" @keydown="form.onKeydown($event)">
				<div class="modal-body">
                    <div class="form-group">
                        ...
                        <input id="product_type_name" type="text" placeholder="Type Name" class="form-control"
                            v-model="form.product_type_name" :class="{ 'is-invalid': form.errors.has('product_type_name') }">
                            <has-error :form="form" field="product_type_name"></has-error>
                    </div>
				</div>
    ...
    var app = new Vue({
        ...
        methods: {
            ...
            storeData(){
                ...
                this.form.post("{{ route('producttype.store') }}")
                    .then(response => {
            ...
            updateData(){
                ...
                url = "{{ route('producttype.update', ':id') }}".replace(':id', this.form.id)
                this.form.put(url)
                    .then(response => {
            ...
            deleteData(id){
                DeleteConfirmation.fire().then((result) => {
                    if (result.value) {
                        url = "{{ route('producttype.destroy', ':id') }}".replace(':id', id)
                        axios.delete(url)
                            .then(response => {
            ...
            refreshDatatable(){
            	axios.get("{{ route('producttype.all') }}")
                    .then(response => {
